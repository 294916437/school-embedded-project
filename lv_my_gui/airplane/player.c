    ...
    for(int j = 0; j < bullet_com; j++)
    {
        ...
        for(int i = 0; i < BULLET_MAX; i++)
        {
            ...
            if(bullet_states[j][i] == OBJ_STATE_FREE)
            {
                ...
                // ========== 第4步：计算弹道类型 ==========
                // judge = 中心位置
                double judge = (bullet_com + 1.0) / 2.0f;  // bullet_com=3时，judge=2.0
                double cnt = (double)(j + 1);               // 当前弹道编号（从1开始）

                // ========== 第5步：根据位置计算角度和坐标 ==========
                if(cnt < judge)  // 左侧弹道
                {
                    // 计算偏离角度（负角度，向左）
                    float angle_rad = (angle_step) * (int)((judge - cnt) / 1) * 3.1415926f / 180.0f;
                ...
                {
                    // 计算偏离角度（正角度，向右）
                    float angle_rad = (angle_step) * (int)((cnt - judge) / 1) * 3.1415926f / 180.0f;
...
void move_bullet_logic() {
    ...
    const float BULLET_SPEED = 10.0f;           // 子弹速度（像素/帧）
    const float ANGLE_TO_RADIAN = PI / 180.0f;  // 角度转弧度系数
    const int SCREEN_WIDTH = 1024;
    const int SCREEN_HEIGHT = 600;
    const int COLLISION_DISTANCE_SQ = 1600;     // 碰撞距离² (40²)
    ...
    for(int j = 0; j < bullet_com; j++) {
        for(int i = 0; i < BULLET_MAX; i++) {
            ...
            for(int e = 0; e < ENEMIES; e++) {
                ...
                if(dx*dx + dy*dy < COLLISION_DISTANCE_SQ) {  // 碰撞！
                    // ========== 第9步：难度递增 ==========
                    ...
                    if(game_score % 700 == 0 && ENEMY_BULLET < ENEMY_BULLET_MAX) {
                        ENEMY_BULLET++;  // 每700分增加敌机连发数
                    }
                    
                    // ========== 第10步：敌机血量处理 ==========
                    if(--enemies_hp[e] == 0) {  // 敌机死亡
                        game_score++;  // 分数+1
                        bullet_states[j][i] = OBJ_STATE_FREE;  // 释放子弹
                        enemy_states[e] = OBJ_STATE_FREE;       // 释放敌机
...
void move_plane(lv_event_t *e) {
    ...
    air_draw *state = lv_event_get_user_data(e);          // 用户数据
    ...
    if (code == LV_EVENT_PRESSED) {
        ...
        // 记录初始触摸位置（绝对坐标）
        state->offset_x = touch_pos.x;
        state->offset_y = touch_pos.y;
        
        // 记录飞机初始位置（从共享数据获取，线程安全）
        state->start_plane_x = game_data.player_x;
        state->start_plane_y = game_data.player_y;
       
        state->is_dragging = true;  // 标记开始拖动
    }
    ...
    // ========== 第3步：处理拖动事件 ==========
        ...
        // 计算触摸点偏移量
        int32_t dx = touch_pos.x - state->offset_x;  // 水平偏移
        int32_t dy = touch_pos.y - state->offset_y;  // 垂直偏移
        
        // 计算新位置（应用灵敏度）
        int32_t new_x = state->start_plane_x + (int32_t)(dx * sensitivity);
        int32_t new_y = state->start_plane_y + (int32_t)(dy * sensitivity);
        
        // 边界限制（飞机60x60，屏幕1024x600）
        new_x = LV_CLAMP(0, new_x, 1024 - 60);  // [0, 964]
        new_y = LV_CLAMP(0, new_y, 600 - 60);   // [0, 540]
        
        // 更新UI显示
        lv_obj_set_pos(state->target, new_x, new_y);
    }
    
    // ========== 第4步：处理释放事件 ==========
    else if (code == LV_EVENT_RELEASED || code == LV_EVENT_CANCEL) {
        state->is_dragging = false;  // 结束拖动
    }
}
