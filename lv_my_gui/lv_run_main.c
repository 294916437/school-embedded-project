...
static void main_grid(void)
{
    static lv_coord_t col_dsc[] = { 110,110,110,110,110, LV_GRID_TEMPLATE_LAST };
    static lv_coord_t row_dsc[] = { 100,110,110,LV_GRID_TEMPLATE_LAST };
...
//---------------------------------视频（替换原音乐功能）-------------------------------------------------------------------------
    ...
    lv_obj_add_event_cb(video_imgbtn,lv_event_video_cb,LV_EVENT_CLICKED,NULL);

    lv_obj_t*label_video1 = lv_label_create(obj);
    lv_obj_set_style_text_font(label_video1,&main_imgfont,LV_PART_MAIN);
    lv_label_set_text(label_video1,"视频");
    lv_obj_set_style_text_color(label_video1,lv_color_hex(0xffffff),LV_PART_MAIN);
    lv_obj_align_to(label_video1,video_imgbtn,LV_ALIGN_OUT_BOTTOM_MID,-10,-5);
...
//---------------------------------游戏-------------------------------------------------------------------------
    ...
    lv_obj_t *game_imgbtn = lv_imgbtn_create(obj);
    lv_imgbtn_set_src(game_imgbtn,LV_IMGBTN_STATE_RELEASED,NULL,&main_control, NULL);
    lv_obj_set_size(game_imgbtn,55,55);
    lv_obj_align(game_imgbtn,LV_ALIGN_CENTER,0,-15);
    lv_obj_add_event_cb(game_imgbtn,lv_event_game_cb,LV_EVENT_CLICKED,NULL);

    lv_obj_t*label_game = lv_label_create(obj);
    lv_obj_set_style_text_font(label_game,&main_imgfont,LV_PART_MAIN);
    lv_label_set_text(label_game,"游戏");
    lv_obj_set_style_text_color(label_game,lv_color_hex(0xffffff),LV_PART_MAIN);
    lv_obj_align_to(label_game,game_imgbtn,LV_ALIGN_OUT_BOTTOM_MID,-10,-5);
...
//--------------------------------------画板---------------------------------------------------------------------
    ...
    lv_obj_set_grid_cell(obj, LV_GRID_ALIGN_STRETCH, 3, 2,
                              LV_GRID_ALIGN_STRETCH, 0, 1);
}
