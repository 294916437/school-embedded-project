...
static void lv_100ask_sketchpad_constructor(const lv_obj_class_t * class_p, lv_obj_t * obj)
{
    ...
    sketchpad->line_rect_dsc.round_start = 1;
    sketchpad->line_rect_dsc.round_end = 1;
...
static void lv_100ask_sketchpad_destructor(const lv_obj_class_t * class_p, lv_obj_t * obj)
{
    ...
    if(sketchpad->draw_buf != NULL) {
        lv_mem_free(sketchpad->draw_buf);
        sketchpad->draw_buf = NULL;
    }
}
...
static void lv_100ask_sketchpad_event(const lv_obj_class_t * class_p, lv_event_t * e)
{
    ...
    if (code == LV_EVENT_PRESSING)
    {
        ...
        {
            lv_point_t p1 = {last_x, last_y};
            lv_point_t p2 = {point.x, point.y};
            
            // 使用 canvas 的绘制线段 API
            lv_canvas_draw_line(obj, &p1, &p2, &sketchpad->line_rect_dsc);
